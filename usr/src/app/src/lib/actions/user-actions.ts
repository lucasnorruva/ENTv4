...
import { getUserById, getCompanies } from '@/lib/auth';
...
export async function createUserAndCompany(
  ...
  // If new user is a developer, create a default API key
  if (newUser.roles.includes(UserRoles.DEVELOPER)) {
    const newApiKey: ApiKey = {
        id: newId('key'),
        label: 'Default Key',
        rawToken: `nor_mock_${userId.slice(0, 4)}_${Math.random().toString(36).substring(2, 10)}`,
        token: `nor_mock_******************${Math.random().toString(36).substring(10, 14)}`,
        status: 'Active',
        userId: userId,
