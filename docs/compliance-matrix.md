...
Note: The above table can be extended. The intent is to map each regulation to relevant DPP fields. For example, RoHS compliance (including supplier declarations and test reports [bradyid.com](https://www.bradyid.com)) corresponds to DPP’s materials and substances sections. The EU DPP framework expects a unique identifier and “compliance documentation” for each law [data.europa.eu](https://data.europa.eu), so in practice each row’s “DPP Data Mapping” indicates which DPP attributes carry the required info.

```jsonc
// Sample JSON Schema snippet for the compliance regulations mapping
{
  "type": "object",
  "properties": {
    "regulations": {
      ...
      "items": {
        ...
        "properties": {
          ...
          "dppMapping": { "type": "string" }
        },
        "required": ["id", "scope", "requiredFields", "productCategories", "dppMapping"]
      }
    }
  }
}
...
```json
{
  "regulations": [
    {
      "id": "RoHS",
      "scope": "Restricts 10 hazardous substances (Pb, Hg, Cd, Cr(VI), PBB, PBDE, phthalates) in EEE",
      "requiredFields": [
        "List of restricted substances present",
        "Supplier declarations",
        "Lab test reports",
        "Material Safety Data Sheets"
      ],
      "productCategories": ["EEE Category 1-11 (electrical/electronic equipment)"],
      "dppMapping": "Map to product materials and 'substances of concern'; include RoHS conformity declarations"
    },
    {
      "id": "REACH",
      "scope": "Regulates chemical substances, requires registration of substances used in products",
      "requiredFields": [
        "Substances in product (names, CAS numbers)",
        "Safety Data Sheets (SDS)",
        "SVHC presence indicator",
        "Authorization status"
      ],
      "productCategories": ["All products containing chemicals"],
      "dppMapping": "Map chemicals and SVHC info to DPP (e.g. 'substances' array), link SDS docs"
    },
    {
      "id": "ESPR",
      "scope": "EU Ecodesign for Sustainable Products – new requirements on durability, reparability, recyclability",
      "requiredFields": [
        "Design attribute data (durability, energy efficiency, etc.)",
        "Bill of materials",
        "Recycling instructions"
      ],
      "productCategories": ["Consumer goods, appliances, electronics, textiles"],
      "dppMapping": "Include product lifespan, repair manual links, material composition in DPP"
    }
...
The JSON schema above defines a regulations array where each entry includes the regulation ID, its scope, required data fields, applicable product categories, and how to map into DPP fields. This schema can be integrated into the DPP platform to validate that all necessary compliance data is captured in the product passport. Sources: EU authorities describe DPP content as including compliance documentation and substance info [data.europa.eu](https://data.europa.eu). Firebase’s security docs explain using JWTs with custom role claims [firebase.google.com](https://firebase.google.com). Solidity’s keccak256 hashing is standard for on-chain anchoring [quicknode.com](https://www.quicknode.com). The EU’s EBSI framework uses W3C Verifiable Credentials for trust [ec.europa.eu](https://ec.europa.eu). The UN/CEFACT DPP spec provides a JSON-LD model for product data [uncefact.github.io](https://uncefact.github.io). Guidance on RoHS compliance confirms the need for supplier declarations and SDS [bradyid.com](https://www.bradyid.com). All these inform the architecture and compliance schema above.
