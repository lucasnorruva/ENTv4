...
export default function GlobalTrackerClient({
  ...
  const handleAnalyzeRoute = useCallback(
    (productId: string, origin: string, destination: string) => {
      startAnalysisTransition(async () => {
        try {
          const result = await analyzeSimulatedTransitRoute(
            productId,
            origin,
            destination,
            user.id,
    ...
    [user.id, products, toast],
  );

  const filteredCountryData = useMemo(() => {
    if (countryFilter === 'all') return MOCK_CUSTOMS_DATA;
    if (countryFilter === 'eu')
      return MOCK_CUSTOMS_DATA.filter(d => EU_COUNTRIES.has(d.region));
  ...
  const polygonColor = useCallback(
    (feat: any) => {
      ...
      if (
        riskFilteredData.some(d =>
          d.keywords.includes(countryName.toLowerCase()),
        )
      ) {
        return theme === 'dark'
          ? 'rgba(56, 189, 248, 0.3)'
    ...
    <div className="absolute inset-0">
      ...
      {clickedCountry && (
        <ClickedCountryInfoCard
          ...
          productsTo={products.filter(
            p => p.transit?.destination === clickedCountry.ADMIN,
          )}
          productsFrom={products.filter(
            p => p.transit?.origin === clickedCountry.ADMIN,
          )}
