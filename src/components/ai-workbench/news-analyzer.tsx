...
import { BrainCircuit, Loader2, Telescope, Newspaper, TrendingUp, TrendingDown, Minus } from 'lucide-react';
...
export default function NewsAnalyzer({ user }: { user: User }) {
  ...
  const handleRunAnalysis = () => {
    if (!newsText.trim()) {
        toast({ title: "No text provided", description: "Please paste one or more news articles to analyze.", variant: "destructive"});
        return;
    }
    setResult(null);
    startTransition(async () => {
      try {
        const analysisResult = await runNewsAnalysis(newsText, user.id);
        setResult(analysisResult);
        toast({
          title: 'Analysis Complete',
          description: 'The news articles have been analyzed for regulatory signals.',
        });
      } catch (error: any) {
        toast({
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          Paste news articles or policy documents below. The AI will extract key takeaways and sentiment regarding future regulatory pressure.
        </CardDescription>
      </CardHeader>
      <CardContent className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="space-y-2">
            <Textarea
                placeholder="Paste news articles or policy documents here..."
                className="h-64"
                value={newsText}
                onChange={(e) => setNewsText(e.target.value)}
            />
             <Button onClick={handleRunAnalysis} disabled={isPending || !newsText.trim()}>
            ...
            {isPending ? (
                <div className="flex flex-col items-center justify-center h-full text-muted-foreground">
                 ...
                 <div className="space-y-4">
                    ...
                    <div>
                        <p className="text-sm font-semibold text-foreground mb-1">Key Takeaways:</p>
