...
export const CalculateSustainabilityInputSchema = z.object({
...
export const AnalyzeProductLifecycleInputSchema = z.object({
  product: AiProductSchema,
});
...
export const AnalyzeProductLifecycleOutputSchema = z.object({
  carbonFootprint: z.object({
    ...
    summary: z
      .string()
      .describe(
        'A brief summary explaining the carbon footprint estimation.',
      ),
  }),
  ...
  highestImpactStage: z
    ...
    .describe(
      'The lifecycle stage with the highest environmental impact.',
    ),
  improvementOpportunities: z
    .array(z.string())
    .describe(
      'A list of suggestions to improve the product lifecycle.',
    ),
});
...
export const SummarizeComplianceGapsInputSchema = z.object({
  product: z.custom<any>(),
  compliancePath: z.custom<any>(),
});
...
export const GenerateQRLabelTextInputSchema = z.object({
  product: AiProductSchema,
});
...
export const ValidateProductDataInputSchema = z.object({
  product: AiProductSchema,
});
...
export const DataQualityWarningSchema = z.object({
  ...
  warning: z
    .string()
    .describe('A description of the potential data anomaly.'),
});
...
// analyze-bom
export const AnalyzeBomInputSchema = z.object({
...
const AnalyzedMaterialSchema = z.object({
  ...
  percentage: z
    .number()
    .optional()
    .describe('The percentage of this material in the product, if specified.'),
  origin: z
    .string()
    .optional()
    .describe('The origin country, if specified.'),
});
...
export const CreateProductFromImageOutputSchema = z.object({
  ...
  category: z
    .enum(['Electronics', 'Fashion', 'Home Goods', 'Construction', 'Food & Beverage'])
    .describe(
...
export const GenerateProductDescriptionInputSchema = z.object({
  ...
  materials: z
    ...
    .describe('A list of key materials.'),
});
...
export const GenerateProductDescriptionOutputSchema = z.object({
  productDescription: z
    .string()
    .describe('The generated product description (2-4 sentences).'),
});
...
const PcdsStatementSchema = z.object({
  property: z
    .string()
    .describe(
      "The specific circularity property (e.g., 'Recycled Content', 'Repairability').",
    ),
  value: z
    .string()
    .describe(
      "The value or state of the property (e.g., '45%', 'High', 'Yes').",
    ),
  methodology: z
    .string()
    .optional()
    .describe(
      "The methodology used to determine the value (e.g., 'ISO 14021').",
    ),
});
export const PcdsOutputSchema = z.object({
  header: z.object({
    ...
    manufacturer: z
      .string()
      .describe('The name of the manufacturer/supplier.'),
    generationDate: z
      .string()
      .describe('The date the PCDS was generated (YYYY-MM-DD).'),
  }),
  statements: z
    .array(PcdsStatementSchema)
    .describe('A list of circularity statements about the product.'),
});
...
export const PredictLifecycleOutputSchema = z.object({
  predictedLifespanYears: z
    .number()
    .describe(
      'The AI-predicted operational lifespan of the product in years.',
    ),
  keyFailurePoints: z
    .array(z.string())
    .describe(
...
export const AnalyzeTextileInputSchema = z.object({
  ...
  dyeProcess: z
    .string()
    .optional()
    .describe('Description of the dyeing process used.'),
});
...
export const AnalyzeTextileOutputSchema = z.object({
  ...
  dyeSafetyAssessment: z
    .string()
    .describe(
      'A brief assessment of the potential risks associated with the described dye process (e.g., "Azo dyes check recommended").',
    ),
});
...
export const AnalyzeElectronicsComplianceOutputSchema = z.object({
  ...
  summary: z
    .string()
    .describe('An overall summary of the compliance findings.'),
});
...
export const AnalyzeConstructionMaterialInputSchema = z.object({
  materialName: z
    .string()
    .describe(
      'The name of the construction material (e.g., "Portland Cement", "Structural Steel").',
    ),
  manufacturingProcess: z
    .string()
    .optional()
    .describe(
      'A brief description of the manufacturing process (e.g., "Blast Furnace").',
    ),
...
export const AnalyzeConstructionMaterialOutputSchema = z.object({
  embodiedCarbon: z.object({
    ...
    unit: z
      .string()
      .default('kgCO2e/kg')
      .describe('The unit for the embodied carbon.'),
    assessment: z
      .string()
      .describe(
        'A brief explanation of the carbon assessment based on the material and process.',
      ),
  }),
  recyclabilityPotential: z
    .enum(['High', 'Medium', 'Low', 'Not Recyclable'])
    .describe(
      'The potential for this material to be recycled at end-of-life.',
    ),
  complianceNotes: z
    .array(z.string())
    .describe(
      'A list of notes regarding potential compliance issues or standards (e.g., "Check local building codes for usage", "May require EPD").',
    ),
});
...
export const ClassifyHsCodeInputSchema = z.object({
  ...
  category: z
    .string()
    .describe(
      'The general category of the product (e.g., "Electronics", "Fashion").',
    ),
  materials: z
    .array(z.object({ name: z.string(), percentage: z.number().optional() }))
    .optional()
    .describe("An optional list of the product's key materials."),
});
export type ClassifyHsCodeInput = z.infer<typeof ClassifyHsCodeInputSchema>;
export const ClassifyHsCodeOutputSchema = z.object({
  code: z
    .string()
    .regex(/^\d{4}\.\d{2}$/, 'HS Code must be in the format XXXX.XX')
    .describe('The 6-digit Harmonized System (HS) code.'),
  description: z
    .string()
    .describe('The official description of the HS code category.'),
  confidence: z
    .number()
    .min(0)
    .max(1)
    .describe('The confidence score of the classification (0.0 to 1.0).'),
});
...
export type GenerateSustainabilityDeclarationInput = z.infer<
  typeof GenerateSustainabilityDeclarationInputSchema
>;
export const GenerateSustainabilityDeclarationOutputSchema = z.object({
  declarationText: z
    .string()
    .describe(
      'The full text of the Sustainability Declaration in Markdown format.',
    ),
});
export type GenerateSustainabilityDeclarationOutput = z.infer<
  typeof GenerateSustainabilityDeclarationOutputSchema
>;
...
export const GenerateComponentTestsInputSchema = z.object({
  componentName: z
    .string()
    .describe('The name of the React component (e.g., "MyButton").'),
  componentCode: z
    .string()
    .describe('The full source code of the React component.'),
});
...
export const GenerateComponentTestsOutputSchema = z.object({
  testCode: z
    .string()
    .describe(
      'The generated test code using Jest and React Testing Library.',
    ),
});
...
export const AnalyzeNewsOutputSchema = z.object({
  keyTakeaways: z
    .array(z.string())
    .describe(
      'A bulleted list of the most important takeaways and signals from the news articles.',
    ),
  sentiment: z
    .enum(['Positive', 'Neutral', 'Negative'])
    .describe('The overall sentiment towards increased regulatory pressure.'),
});
...
export const PredictRegulationChangeInputSchema = z.object({
  signals: z
    .array(z.string())
    .describe(
      'A list of key signals and takeaways from various sources (news, policy papers, etc.).',
    ),
  targetIndustry: z
    .string()
    .describe('The industry to focus the prediction on.'),
});
export type PredictRegulationChangeInput = z.infer<
  typeof PredictRegulationChangeInputSchema
>;
export const PredictRegulationChangeOutputSchema = z.object({
  prediction: z
    .string()
    .describe(
      'A single, specific prediction about a potential new regulation or a change to an existing one.',
    ),
  confidence: z
    .number()
    .min(0)
    .max(1)
    .describe('The confidence level of the prediction (0.0 to 1.0).'),
  timeframe: z
    .enum(['6-12 months', '12-24 months', '24+ months'])
    .describe('The likely timeframe for the predicted change.'),
  impactedRegulations: z
    .array(z.string())
    .describe(
      'A list of existing regulations that might be amended or replaced.',
    ),
  proactiveSteps: z
    .string()
    .describe(
      'A single, high-impact proactive step a company should consider taking in response to this prediction.',
    ),
});
