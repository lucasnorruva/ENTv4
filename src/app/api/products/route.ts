...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    await logAuditEvent('api.post', newProduct.id, { endpoint: '/api/products', status: 201 }, user.id);
    return NextResponse.json(newProduct, { status: 201 });
  } catch (error: any) {
    // Check if it's a Zod validation error
    if (error.name === 'ZodError') {
      await logAuditEvent('api.post', 'N/A', { endpoint: '/api/products', status: 400, error: 'Invalid data' }, user.id);
    ...
    await logAuditEvent('api.post', 'N/A', { endpoint: '/api/products', status: 500, error: 'Internal Server Error' }, user.id);
    return NextResponse.json(
